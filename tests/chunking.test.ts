import { chunkNoteToCaptions } from "../src/chunking";

describe("chunkNoteToCaptions", () => {
  it("returns empty array for undefined", () => {
    expect(chunkNoteToCaptions(undefined)).toEqual([]);
  });

  it("returns empty array for empty string", () => {
    expect(chunkNoteToCaptions("")).toEqual([]);
  });

  it("returns empty array for whitespace only", () => {
    expect(chunkNoteToCaptions("   \n  \n  ")).toEqual([]);
  });

  it("returns single chunk for single line", () => {
    expect(chunkNoteToCaptions("Hello world, this is a test")).toEqual([
      "Hello world, this is a test",
    ]);
  });

  it("splits multi-line notes into one chunk per line", () => {
    const note = "This is the first line\nThis is the second line\nThis is the third line";
    expect(chunkNoteToCaptions(note)).toEqual([
  ...
  it("recognizes [click:N] markers and removes them", () => {
    ...
    expect(chunkNoteToCaptions(note)).toEqual(["Part one of the note", "Part two of the note"]);
  });

  it("handles case-insensitive click markers", () => {
    const note = "Before the marker\n[CLICK]\nAfter the marker";
    expect(chunkNoteToCaptions(note)).toEqual(["Before the marker", "After the marker"]);
  });
  ...
  it("limits output with maxChunksPerSlide", () => {
    ...
    expect(chunkNoteToCaptions(note, { maxChunksPerSlide: 3 })).toEqual([
      "First long line here",
      "Second long line here",
      "Third long line here",
    ]);
  ...
  it("wraps long lines at word boundary", () => {
    ...
    const result = chunkNoteToCaptions(note, { maxCharsPerLine: 50 });
  ...
  it("merges short chunks with adjacent", () => {
    ...
    const result = chunkNoteToCaptions(note, { minCharsPerChunk: 10 });
  ...
  it("does not split by line breaks when preferManualLineBreaks is false", () => {
    ...
    const result = chunkNoteToCaptions(note, {
  ...
  it("filters empty lines between content", () => {
    ...
    expect(chunkNoteToCaptions(note)).toEqual(["First line of text", "Second line of text"]);
  });
});
